...
class Promotion extends AuthController{
    ...
    public function index(){
        ...
        if(!empty($price)){
            foreach($price as $item){
                if($item['price'] != $prices['price']){
                    $params['price'] = $prices['price'];
                    $params['ticheng'] = DigitalHelper::mul($prices['price'],DigitalHelper::div($prices['rebate'],100));
                    $params['updatedAt'] = time();
                    Db::name('AgentPrice')->where(['id'=>$item['id']])->update($params);
                    $price = Db::name('AgentPrice')->where(['uid'=>Session('user_id'),'product_type' => Mapper::PRODUCT_TYPE])->order('createdAt desc')->select();
                }
        	}
        }
        return $this->fetch('index',['data'=>$price,'prices'=>$prices]);
    }
    ...
    public function code_img($pid){
        ...
        $img = Imgdeal::getInstance()->img_conversion($platform_img,$agent_price['url'],$code_img,5,$logo,210,1000);

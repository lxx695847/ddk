...
class PromotionLogicQuery extends LogicQuery{
    ...
    public function price($params){
        try{
            ...
            $price = Db::name('AgentPrice')->where('uid',$params['uid'])->find();

            if(!empty($price)){
                throw new Exception('请不要重复添加',1);
            }

            //判断用户是否添加代理
            $authAgent = Db::name('authAgent')->where(['aid'=>$user['agent_class'],'pid'=>$params['type']])->find();
            if(empty($authAgent)){
                throw new Exception('请联系客服添加代理',1);
            }
            ...
            $data = [
                ...
                'price' => $authAgent['price'],
                'ticheng' => DigitalHelper::mul($authAgent['price'],$prices),
                'uid' => $params['uid'],
                'link_name' => $params['uid'],//$params['link_name'],
                'define' => 1,
                'isdel' => 1,
                'createdAt' => time(),
            ];
            ...
            if($id){
                if(isset($params['type']) && $params['type'] == Mapper::PRODUCT_TYPE_TWO){
                    ...
                    $url = $host.'/index/index/index?sign='.$sign;//$this->shortenSinaUrl($host.'/index/index/index?sign='.$sign);
     ...
     * @throws \Exception
     */
        public function prices($params = null){
            try{
                if(empty($params)){

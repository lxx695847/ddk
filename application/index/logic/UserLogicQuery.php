...
class UserLogicQuery extends LogicQuery
{
     ...
     * @throws \Exception
     */
    public function find_one($params = null,$select = '*'){
        try{
            $user = Db::name('user')->where($params)->field($select)->find();
        }catch(\Exception $e){
            $this->log($e);
            throw $e;
        }
    ...
    public function edit($params = null){
        try{
            ...
            if(isset($params['type']) && $params['type'] == 1){
                $result = $this->validate($params,'Zfb');
            }else{
                $result = $this->validate($params,'Bank');
            }
            if($result !== true){
                throw new Exception($result,0);
            }
            if(isset($params['type']) && $params['type'] == 1){
                $data['wname'] = $params['wname'];
                $data['bankname'] = $params['bankname'];
            }else{
                $data['bname'] = $params['bname'];
                $data['banknumber'] = $params['banknumber'];
            }
            $data['phone'] = $params['phone'];
            $status = Db::name('user')->where('id',session('user_id'))->update($data);
            if(!$status){

        ...
        try{
            ...
            if(empty($params['pid'])){
                if(isset($agentid) && !empty($agentid)){
                    $price = Db('AgentPrice')->field('id,price,a_p_id')->where('uid','=',$agentid)->where('product_type','=',Mapper::PRODUCT_TYPE)->where('isdel','=',1)->order('createdAt desc')->find();
                }else{
                    $price = Db('AgentPrice')->field('id,price,a_p_id')->where('id','=',Mapper::PID_PINGTTAI_ID)->where('product_type','=',Mapper::PRODUCT_TYPE)->where('isdel','=',1)->find();
                }
            }else{
            	 $price = Db::name('AgentPrice')->field('id,price,a_p_id')->where(['id'=>$params['pid']])->find();
            }
            ...
            if(!isset($params['mobile']) && empty($params['mobile'])){
             ...
             //判断用户是否添加代理
            $authAgent = Db::name('authAgent')->where(['id'=>$price['a_p_id'],'pid'=>Mapper::PRODUCT_TYPE])->find();

            if(!empty($authAgent)){
                if($price['price'] != $authAgent['price']){
                    $datas['price'] = $authAgent['price'];
                    $datas['ticheng'] = DigitalHelper::mul($authAgent['price'],DigitalHelper::div($authAgent['rebate'],100));
                    $datas['updatedAt'] = time();
                    Db::name('AgentPrice')->where(['id'=>$price['id']])->update($datas);
                    $price = Db::name('AgentPrice')->where(['id'=>$price['id']])->find();
                }
            }
            //用户ID

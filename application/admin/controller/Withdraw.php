...
class Withdraw extends AdminBase
{
      ...
      public function index($keyword = '', $page = 1,$date1='',$date2='')
    {
		...
		foreach($withdraw_list as &$item){
			  $w = $this->withdraw_model->field('id,user_id,bankcard')->where('user_id',$item['user_id'])->where('status',1)->select();
			  $list = collection($w)->toArray();
			  $arr = array_column($list,'bankcard');
				  if(isset($arr) && count($arr) > 0){
			  	 if(!in_array($item['bankcard'],$arr)){
					$item['wi_type'] = 1;	
				}
			  }
		}
  ...
  public function add()
    ...
    public function save()
    {
        if ($this->request->isPost()) {
            ...
            $user = db('user')->field('id,money')->where('id',$data['user_id'])->find();
            
            if(isset($data['money']) && $data['money'] > $user['money']){
            	$this->error('提现金额不能大于用户余额');
            }

...
class Chaxun extends AdminBase
{
    ...
    public function xiangqing()
    {
		...
		$json=db("user")->where("id",$chaxun['uid'])->value("json");
    	...
    	if($product['a_g_id'] == 7){
            ...
            if($data['bairo']){
				if($data['bairo']['tianyan_mobile']){
					if($data['bairo']['tianyan_mobile']=="Failure"){
						$result = "Failure";
						$send['id']=$data['bairo']['id'];
						$send['status']="Failure";
					} elseif ($data['bairo']['tianyan_mobile']=="Crawling"){
						$result = "Crawling";
						$send['id']=$data['bairo']['id'];
						$send['status']="Crawling";
					}else{
						$result = json_decode($data['bairo']['tianyan_mobile'],true);
						$createAt = !empty($data['chaxun']['dates']) ? date('Y-m-d H:i', $data['chaxun']['dates']) : date('Y-m-d H:i', $data['sales']['createAt']);
						$this->assign("createAt",$createAt);
						$send['id']=$data['bairo']['id'];
						$send['status']="Success";
					}
				}else{
    ...
    public function case_data($data)
    {
        ...
        for($i=1; $i <= 10; $i++){
            ...
            if(isset($data['ex_execut'.$i.'_name']) && !empty($data['ex_execut'.$i.'_name'])){
                $result['executive'][$i]['name'] = substr_replace($data['ex_execut'.$i.'_name'], '**', 3, 6); //姓名
            ...
            if(isset($data['ex_bad'.$i.'_name']) && !empty($data['ex_bad'.$i.'_name'])){
                ...
                $result['unt'][$i]['name'] =  substr_replace($data['ex_bad'.$i.'_name'], '**', 3, 6); // //姓名
                ...
                $result['unt'][$i]['time'] = date('Y年m月d日',substr($data['ex_bad'.$i.'_time'],0,10));;//time
                ...
                $result['unt'][$i]['posttime'] = date('Y年m月d日',substr($data['ex_bad'.$i.'_posttime'],0,10));; //失信案件发布时间
